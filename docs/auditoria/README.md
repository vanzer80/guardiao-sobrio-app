...
- ✅ **Achado 1 — `Gerar código` CORRIGIDO (causa-raiz real, Rodada 4).** A hipótese inicial (RLS/plano) foi **refutada** na execução: a coluna `family_connections.invitation_expires_at` nunca foi aplicada ao banco (migration drift) → `createInvite` falhava com *column does not exist* antes da RLS. Corrigido (coluna + índice + policy RESTRICTIVE + `effective_plan()`); validação e2e pendente. Ver `04-relatorio-rodada4-execucao.md`.
