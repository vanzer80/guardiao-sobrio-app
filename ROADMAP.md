...
| DT9 / DRIFT-01 | **DIAGNÓSTICO CONCLUÍDO (21/06/2026).** Migration `20260619203119` registrada mas DDL não executado: faltam `profiles.plan`, `profiles.stripe_customer_id`, `subscriptions.stripe_subscription_id` e possivelmente `subscription_audit_log`. 3 migrations RLS (`20260621000000/090000/170000`) aplicadas via API mas não registradas em `schema_migrations` → risco de conflito em `supabase db push`. Ver `docs/auditoria/07-diagnostico-drift-resultados.md`. | `supabase/migrations/` vs DB real | **Alto** — resolve antes de qualquer sprint de monetização | Criar migration idempotente com as 3 colunas + audit_log + registrar migrations RLS |
| DT10 / MO-07 | **DIAGNÓSTICO CONCLUÍDO — QUEBRADO (21/06/2026).** Cliente lê `profiles.plan` (coluna ausente → sempre `undefined` → plan = 'free'). Webhook Stripe falha silenciosamente em todos os eventos (colunas inexistentes → 400 ignorado; audit_log → provável 500). Usuários pagantes via Stripe veem plano 'free' no app. Único path funcional: trial via `profiles.trial_end`. Ver `docs/auditoria/07-diagnostico-drift-resultados.md`. | `app/_layout.tsx:120` · `supabase/functions/handle-stripe-webhooks/index.ts` | **Alto** — monetização completamente não-funcional para usuários Stripe | Corrigir após DT9: restaurar colunas + corrigir webhook (`provider_subscription_id` vs `stripe_subscription_id`) + validar `_layout.tsx` |
